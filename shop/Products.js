	...
	static navigationOptions = ({ navigation }) => {		
		return {
			title: "Products",
			 headerRight: (
				<Button onPress={() => {
					console.log("Open Add Product screen :: ");
					navigation.navigate('Product', { productId: 1, title: "Add Product" });
				}} title="Add" color="#000">Add</Button>
			),	
		}
	};
					...
					renderItem={({item}) => (	
						<TouchableOpacity
							onPress={() => {
								...
								this.props.navigation.navigate('Category', { categoryId: item.id });
							}}>
							<Text >{item.name}</Text>
						</TouchableOpacity>
					)}
